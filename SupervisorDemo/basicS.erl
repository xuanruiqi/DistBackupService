...
loop(Pid, Argument) ->
    X = rand:uniform(500000000),
    case X of
	10 -> erlang:display('Crashing: '),
...
%%--------------------------------------------------------------------
start_link(Argument) ->
    gen_server:start_link({local, Argument}, ?MODULE, [Argument], []).
...
%%--------------------------------------------------------------------
init([Argument]) ->
    erlang:display('Starting up new server: '),
    erlang:display(Argument),
    spawn(basicS, loop, [self(), Argument]),
